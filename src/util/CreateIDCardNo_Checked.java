 ...
 * 身份证号验证类
 */
public class CreateIDCardNo_Checked {
	/**
	 ...
	 * @throws Exception
	 */
	public CreateIDCardNo_Checked(String cardId) throws Exception {
	...
	public static String genCardId(int num, String filePath, int yearLen) {
		...
		try {
			...
			// 读取properties
			InputStream file = CreateIDCardNo_Checked.class.getClassLoader().getResourceAsStream(propFile);
			// 加载properties文件
	...
	/**
	 * 
	 * @param age 年龄  不是特别准确  比如你写个10岁  可能是10岁 也可能是11 
	 * 					
	 * @return
	 */
	public static String genOneCardId( int age) {
		String ret = "";
		try {
...
//			int size = code.length;
			...
			Random random = new Random();
			for (int i = 0; i < num; i++) {
				String areaCode = "650101";
				Date   curDate   =   new   Date(System.currentTimeMillis());
				@SuppressWarnings("deprecation")
				SimpleDateFormat sdf=new SimpleDateFormat("yyyy");
				int year = Integer.parseInt(sdf.format(curDate)) - age-1;
				int month = random.nextInt(11);
				if (month == 0)
					month = 12;
				int day = 0;
...
//			e.printStackTrace();
//		}
		System.out.println(genOneCardId(10));
		System.out.println(genOneCardId(20));
		System.out.println(genOneCardId(30));
		System.out.println(genOneCardId(40));
		System.out.println(genOneCardId(50));
